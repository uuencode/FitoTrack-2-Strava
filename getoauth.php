...
if(isset($_GET['code']) && strlen($_GET['code'])==40){
	...
	if(isset($res['access_token']) && isset($res['refresh_token']) && isset($res['expires_at']) && isset($res['athlete']['id'])){
		...
		$fitotrack_url=str_replace('getoauth.php','',$this_url).'?'.$athlete_id.'z'.$hash;
		print '<html><body><p>FitoTrack - Settings - Database - Workout GPX Exports - Click <b style="font-size:120%">+</b> and select <b>HTTP POSTS request</b></p>';
		print '<pre>URL: <b>'.$fitotrack_url.'</b></pre></body></html>';
		die();
	}
...
$auth_url = 'https://www.strava.com/oauth/authorize?client_id='.$strava_client_id.'&response_type=code&redirect_uri='.urlencode($this_url).'&approval_prompt=force&scope=activity%3Awrite';
